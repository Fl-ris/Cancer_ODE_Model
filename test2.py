...
class tumorODE:
    ...
    def hooke_jeeves(self, model_func, params, data_ts, data_vs, methode="rk4",
                     tol=1e-6, alpha_up=1.2, alpha_down=0.5, max_iter=1000):
        """
    ...
    def toon_grafiek(self, titel="Tumor Groei Simulatie"):
...
if __name__ == "__main__":
    ...
    modellen_lijst = [
        ...
        (modeler.mendelsohn_model, {'c': 0.5, 'd': 0.8}),
    ...
    for model_func, start_params in modellen_lijst:
            ...
            print(f"{res['model_naam']:<35}, {res['mse']:.2f}, {res['BIC']:.2f}")

    resultaten.sort(key=lambda x: x['BIC'])
    ...
    winnaar_func = beste_resultaat['functie']
    winnaar_params = beste_resultaat['best_params']
    ts, vs = winnaar_func(**winnaar_params)
    
    modeler.plot(ts, vs, color="red", label=f"Fit: {beste_resultaat['model_naam']}")

    modeler.toon_grafiek()